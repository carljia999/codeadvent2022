...
sub dump_map {
    ...
    say join('', '#.', '#' x $maxx);
    for my $z (1..$maxy) {
        say join('', '#', map { exists $map->{"$_,$z"} ? 'B' : '.' } (1..$maxx)) . '#';
    }
    say join('', '#' x $maxx, '.#');
}
...
sub find_path_dfs {
    my ($start, $end, $minutes) = @_;
    my ($min, $min_path) = (1000,);
...
BAILOUT:
    while (my $s = shift @queue) {
        ...
        {   # Estimate the min minutes by assuming that we can go one step closer.
            ...
            my $estimate = $s->{min} + abs($end->x - $loc->x) + abs($end->y - $loc->y);
        ...
        if ($end->x == 1) {
            @neighbours = reverse @neighbours;
            #say "checking point: $locstr, min: " . $s->{min};
        }
...
# trip 1, from top to bottom
my $min = 1 + find_path_dfs(Point->new(1,1), Point->new($maxx,$maxy), 1);

# trip 2, from bottom to top
$min = 1 + find_path_dfs(Point->new($maxx,$maxy), Point->new(1,1), $min+1);

# trip 3, from top to bottom
$min = 1 + find_path_dfs(Point->new(1,1), Point->new($maxx,$maxy), $min+1);
