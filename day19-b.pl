...
sub simulate_dfs {
    ...
    while (my $s = shift @queue) {
        ...
        my $estimate = $s->esitmate;
        next if $estimate <= $max;
        ...
        for my $r (qw(geode obsidian clay ore)) {
            ...
            if ($r ne 'geode' && $s->robot($r) > $bp->robot_limit($r)) {
                next;
            }
        ...
        @robot_moves = grep {!exists $queue{$_->key}} @robot_moves;
        unshift @queue, @robot_moves;
        $queue{$_->key} = 1 for @robot_moves;
    }
...
sub max_per_state {
    ...
    my $max = max map { max_per_state($bp, $_) } @robot_moves;
